...
run([a.cmake.resolve(),'-S',sources['jxl'],'-B',jb,*common,*['-DJPEGXL_ENABLE_'+n+'=OFF' for n in disabled],'-DJPEGXL_BUNDLE_LIBPNG=OFF','-DJPEGXL_ENABLE_SKCMS=ON','-DJPEGXL_ENABLE_TRANSCODE_JPEG=ON','-DJPEGXL_ENABLE_BOXES=ON','-DJPEGXL_FORCE_SYSTEM_BROTLI=OFF','-DPROVISION_DEPENDENCIES=OFF','-DCMAKE_HAVE_LIBC_PTHREAD=1','-DCMAKE_CXX_FLAGS=-DHWY_COMPILE_ONLY_SCALAR -ffile-prefix-map='+work.as_posix()+'=.'])
