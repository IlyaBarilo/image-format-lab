...
cache=a.cache.resolve() if a.cache else work/'emscripten-cache'
temporary=cache/'tmp';temporary.mkdir(parents=True,exist_ok=True)
env=dict(os.environ,EMSDK=str(sdk),EMSDK_PYTHON=sys.executable,EM_CONFIG=str(sdk/'.emscripten'),EM_CACHE=str(cache),TMP=str(temporary),TEMP=str(temporary),TMPDIR=str(temporary),SOURCE_DATE_EPOCH='1758067200')
...
with (sources['jxl']/'lib/jxl/decode.cc').open('ab') as decoder:
 decoder.write(b'\n')
 decoder.write((package/'block-map.inc').read_bytes())
...
run([a.cmake.resolve(),'-S',sources['jxl'],'-B',jb,*common,*['-DJPEGXL_ENABLE_'+n+'=OFF' for n in disabled],'-DJPEGXL_VERSION=0.12.0','-DJPEGXL_BUNDLE_LIBPNG=OFF','-DJPEGXL_ENABLE_SKCMS=ON','-DJPEGXL_ENABLE_TRANSCODE_JPEG=ON','-DJPEGXL_ENABLE_BOXES=ON','-DJPEGXL_FORCE_SYSTEM_BROTLI=OFF','-DPROVISION_DEPENDENCIES=OFF','-DCMAKE_HAVE_LIBC_PTHREAD=1','-DCMAKE_CXX_FLAGS=-DHWY_COMPILE_ONLY_SCALAR -ffile-prefix-map='+work.as_posix()+'=.'])
run([a.cmake.resolve(),'--build',jb,'--clean-first','--parallel',a.jobs,'--target','jxl'])
if a.libraries_only: raise SystemExit(0)
exports=['malloc','free','viewer_modern_encode','viewer_modern_encode16','viewer_modern_decode','viewer_modern_jpeg_to_jxl','viewer_modern_jxl_to_jpeg','viewer_modern_clear','viewer_modern_error','viewer_modern_output','viewer_modern_output_size','viewer_modern_width','viewer_modern_height','viewer_modern_depth','viewer_modern_block_owners','viewer_modern_block_count']
out=a.output.resolve();out.parent.mkdir(parents=True,exist_ok=True)
run([sys.executable,em/'em++.py',package/'bridge.cpp',jb/'lib/libjxl.a',jb/'lib/libjxl_cms.a',jb/'third_party/brotli/libbrotlienc.a',jb/'third_party/brotli/libbrotlidec.a',jb/'third_party/brotli/libbrotlicommon.a',jb/'third_party/highway/libhwy.a',wb/'libwebp.a',wb/'libsharpyuv.a','-I'+str(sources['jxl']/'lib/include'),'-I'+str(jb/'lib/include'),'-I'+str(sources['webp']/'src'),'-O3','-std=c++17','--no-entry','-sMODULARIZE=1','-sEXPORT_NAME=ViewerModernModule','-sSINGLE_FILE=1','-sSINGLE_FILE_BINARY_ENCODE=0','-sENVIRONMENT=worker,node','-sALLOW_MEMORY_GROWTH=1','-sMALLOC=emmalloc','-sEMIT_EMSCRIPTEN_LICENSE=1','-sINITIAL_MEMORY=33554432','-sMAXIMUM_MEMORY=1073741824','-sSTACK_SIZE=1048576','-sFILESYSTEM=0','-sDYNAMIC_EXECUTION=0','-ffile-prefix-map='+package.as_posix()+'=modern-sources','-Wl,-Map='+str(work/'modern-codecs.map'),'-sEXPORTED_FUNCTIONS='+json.dumps(['_'+n for n in exports]),'-sEXPORTED_RUNTIME_METHODS='+json.dumps(['HEAPU8','UTF8ToString']),'-o',out])
