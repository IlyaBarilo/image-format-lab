// UTIF 3.1.0, Copyright (c) 2017 Photopea. MIT; see UTIF-LICENSE.
...
UTIF.decode._decompress = function(img,ifds, data, off, len, cmpr, tgt, toff, fo)  // fill order
{
	...
	//var time = Date.now();
	if(false) {}
	else if(cmpr==1 || (len==tgt.length && cmpr!=32767 && cmpr!=6 && cmpr!=7)) for(var j=0; j<len; j++) tgt[toff+j] = data[off+j];
...
UTIF.toRGBA8 = function(out)
{
	...
	else if(intp==5) 
	{
		...
		for(var i=0; i<area; i++) {
			...
			img[qi]=~~(C*K+0.5);  img[qi+1]=~~(M*K+0.5);  img[qi+2]=~~(Y*K+0.5);  img[qi+3]=gotAlpha ? data[si+4] : 255;
		}
	}
