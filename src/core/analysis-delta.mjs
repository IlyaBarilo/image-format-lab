...
export function analysisDelta(items, settings) {
  ...
  const [a, b] = items.map(item => item.data), density = type === 'waveform' || type === 'parade';
  ...
  const bins = type === 'histogram' ? 256 : type === 'profile' ? Math.max(a.bins, b.bins) : Math.max(a.columns, b.columns);
  if (!Number.isInteger(bins) || bins < 1 || bins > (type === 'profile' ? 1024 : 256)) throw new Error('Некорректная сетка графика.');
  ...
  return { type, operation: 'second-minus-first', pair: items.map(item => item.cell),
    ...
    alignment: type === 'histogram' ? 'same-level' : density ? 'relative-column-area' : 'relative-position-linear-means',
    bins, ...(density ? { columns: bins, rows: 256 } : {}), maximum, channels };
}
