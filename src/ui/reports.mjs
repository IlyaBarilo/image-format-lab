...
export function createReports({app, els}, deps) {
  ...
  function saveComparisonReport(type) {
    ...
    const headers = ["source","source_width","source_height","source_bytes","created_at","browser","cell","status","format","quality","palette","dither","matte","metadata","codec","bytes","width","height","percent_of_source","psnr_rgb_db_white_background","alpha_mean_error_percent","processing_ms","error"];
    const rows = report.variants.map(v => {const m=v.metrics || {};return [report.source.name,report.source.width,report.source.height,report.source.bytes,report.createdAt,report.browser,v.cell,v.status,v.config.format,v.config.quality,v.config.gifColors,v.config.gifDither,v.config.matte,v.config.metadataPolicy || els.metadataPolicy.value,v.codec,m.bytes,m.width,m.height,m.percentOfSource,m.psnrRGB,m.alphaErrorPercent,m.processingMs,v.error];});
