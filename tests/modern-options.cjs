...
(async () => {
  ...
  assert.deepEqual(normalizeModernOptions(), { webpMethod: 4, jxlEffort: 5, jxlDepth: 'auto' });
  assert.deepEqual(normalizeModernOptions({ webpMethod: 0, jxlEffort: 10, jxlDepth:'16' }), { webpMethod: 0, jxlEffort: 10, jxlDepth:'16' });
  for (const options of [{ webpMethod: -1 }, { webpMethod: 7 }, { webpMethod: 2.5 },
    { jxlEffort: 0 }, { jxlEffort: 11 }, { jxlEffort: '5' }, {jxlDepth:'12'}])
  ...
  comparison.variants[2].jxlDepth = '16';
  ...
  assert.equal(restored.variants[2].jxlDepth, '16');
  ...
  assert.equal(normalizeBatchSettings({jxlDepth:'16'}).jxlDepth,'16');
  assert.equal(normalizeBatchSettings({jxlDepth:'12'}).jxlDepth,'auto');
  ...
  const report = { source: { name: 'x.png', width: 1, height: 1, bytes: 10, bitDepth: 8 },
    createdAt: 'test', browser: 'test', variants: [
      ...
      { cell: 2, status: 'ready', codec: 'test', error: null, config: reportFormatConfig({ format: 'jxlLossless', jxlEffort: 10, jxlDepth:'16' }), metrics: {} }
    ] };
  ...
  assert.equal(rows[2][rows[0].indexOf('jxl_depth')],'16');
