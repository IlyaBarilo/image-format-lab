...
(async () => {
  ...
  app.source.pixelBuffer=exact;
  get('analysisType').value='vectorscope';ui.updateAnalysis();await tick();
  const vectorJob=jobs.shift();
  assert.equal(vectorJob.kind,'vectorscope');
  assert.equal(vectorJob.payload.pixelBuffer,exact);
  assert.equal('imageData' in vectorJob.payload,false);
  get('analysisType').value='difference';ui.updateAnalysis();
  vectorJob.resolve({});await tick();
  const differenceJob=jobs.shift();
  assert.equal(differenceJob.kind,'difference');
  assert.equal(differenceJob.payload.pixelBuffer,exact);
  assert.equal(differenceJob.payload.reference,exact);
  assert.equal('imageData' in differenceJob.payload,false);
  ui.collapseAnalysis();differenceJob.resolve({});await tick();
  console.log('PASS vectorscope and difference receive exact cell and source pixels through the analysis scheduler');
