...
(async()=>{
  ...
  const deep=value=>computeLineProfile({width:1,height:1,data:Uint16Array.from([value,value,value,65535]),sampleType:'uint16',bitDepth:16,colorSpace:'srgb'});
  const deepProfile=deep(32769);
  assert.equal(deepProfile.scaleMax,65535);assert.equal(deepProfile.channels[0].min[0],32769);
  assert.equal(deepProfile.channels[0].mean[0],32769);assert.equal(deepProfile.channels[4].mean[0],32769);
  const alpha16=computeLineProfile({width:1,height:1,data:Uint16Array.from([32769,0,0,32768]),sampleType:'uint16',bitDepth:16},'black');
  assert.equal(alpha16.channels[0].mean[0],16385);assert.equal(alpha16.channels[3].mean[0],32768);
  const groupedData=new Uint16Array(1025*4);
  for(let i=0;i<1025;i++)groupedData[i*4+3]=65535;
  groupedData[0]=32768;groupedData[4]=32769;
  const grouped16=computeLineProfile({width:1025,height:1,data:groupedData,sampleType:'uint16',bitDepth:16});
  assert.equal(grouped16.bins,1024);assert.equal(grouped16.channels[0].min[0],32768);
  assert.equal(grouped16.channels[0].max[0],32769);assert.equal(grouped16.channels[0].mean[0],32768.5);
  assert.equal(analysisDelta(pair(deep(32768),deep(32769)),ps).channels[0].values[0],1,'one 16-bit code survives subtraction');
  const mixedProfile=analysisDelta(pair(profile([128]),deep(32769)),ps);
  assert.equal(mixedProfile.unit,'percentage-points');
  assert.ok(Math.abs(mixedProfile.channels[0].values[0]-(32769/65535-128/255)*100)<1e-10);
  const normalizedEqual=analysisDelta(pair(profile([255]),deep(65535)),ps);
  assert.equal(normalizedEqual.maximum,0);
  global.devicePixelRatio=1;
  const {renderAnalysisOverlay,renderAnalysisDelta:renderDeltaProfile}=createAnalysisCombined();
  const mixedCanvas=recordingCanvas();renderAnalysisOverlay(mixedCanvas,pair(profile([255]),deep(65535)),ps);
  assert.equal(mixedCanvas.dataset.yMax,'100');
  const deepCanvas=recordingCanvas();renderAnalysisOverlay(deepCanvas,pair(deep(0),deep(65535)),ps);
  assert.equal(deepCanvas.dataset.yMax,'65535');
  const normalizedCanvas=recordingCanvas();renderDeltaProfile(normalizedCanvas,normalizedEqual,ps);
  assert.equal(normalizedCanvas.dataset.unit,'percentage-points');
  assert.ok(normalizedCanvas.ops.some(op=>op.kind==='text'&&op.text==='Δ п.п.'));
  assert.throws(()=>computeLineProfile({width:1,height:1,data:new Float32Array([0,0,0,1]),sampleType:'float32'}),/float32/);
  ...
  global.devicePixelRatio=1;
  const chromaCanvas=recordingCanvas();renderAnalysisDelta(chromaCanvas,chroma,{type:'ycbcrParade'});
  assert.deepEqual(chromaCanvas.ops.filter(op=>op.kind==='text'&&['Y′','Cb','Cr'].includes(op.text)).map(op=>op.text),['Y′','Cb','Cr']);
