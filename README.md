...
3. Нажмите **Образец** для первого знакомства, стрелку рядом — для выбора других
   встроенных примеров, или **Добавить** для своих изображений.
...
| Исследовать цвет и яркостный сигнал | Панель «Анализ»: гистограммы RGB/alpha/Y′CbCr, Waveform яркости и RGB, RGB/Y′CbCr Parade, вектороскоп и плотностная диаграмма CIE xy для текущих 2/4 ячеек; CIE xy может дополнительно показать цвета исходника с поддерживаемым ICC до ограничения sRGB |
| Проверить границы и градиенты | Профиль RGB/Y′/alpha вдоль общей линии; пространственная карта точных кодов 0 и максимума для RGB/alpha с количеством и долей пикселей. Граничный код сам по себе не доказывает клиппинг |
| Найти отличия в изображении | Карты различий, гистограмма и профиль попиксельных ошибок RGB/alpha с исходником; MAE, RMSE, одномасштабный SSIM и средняя цветовая разница ΔE00 по точным RGBA8/16 пикселям, выбор области для анализа |
...
| PNG | PNG8 — браузер, при RGB ICC — собственное точное чтение; PNG16 — точное чтение серых/RGB/RGBA, включая Adam7 | Полные цвета: Авто / 8 / 16 бит на канал, PNG16 в исходном размере; палитра: 2–256 цветов, дизеринг, двоичная прозрачность, индексы 1/2/4/8 бит; PNG opt: UPNG/pako, 8 бит на канал |
...
| JPEG XL | Встроенный libjxl 0.12.0; точное чтение целочисленного RGBA16 | **JPEG XL** с качеством 1–100 (8 бит) или отдельный **JPEG XL lossless** с выбором Авто / 8 / 16 бит на канал; 16 бит — в исходном размере. Усилие кодирования 1–10. В «Экспериментах» доступно отдельное преобразование исходного JPEG в JXL с побайтовым восстановлением JPEG |
| JPEG 2000 | Встроенный OpenJPEG 2.5.4: JP2 и J2K, 8/16-битные серые/RGB/RGBA | JP2 или J2K: качество 100 — без потерь, 1–99 — с потерями; точные 16 бит сохраняются без изменения размера |
...
| TIFF | Встроенный libtiff: TIFF/BigTIFF, раздельные каналы, ориентация; точное чтение целых TIFF16 для серого/RGB/RGBA с полосами без сжатия, Deflate или PackBits; UTIF / libjpeg-turbo для JPEG-in-TIFF и lossless JPEG | RGBA8: без сжатия, Deflate, LZW или PackBits. RGBA16 в исходном размере: без сжатия или Deflate, с предиктором по выбору |
...
AVIF, JPEG XL, JPEG 2000 и WebP lossless работают автономно. Поддержка всех разновидностей
TIFF/HEIC/JPEG XL/JPEG 2000 не гарантируется; кодеки запускаются при открытии HTML.
PNG16, поддерживаемый TIFF16, JPEG XL lossless и JPEG 2000 без потерь сохраняют 16-битные отсчёты
используемого для кодирования растра. При ICC анализ использует SDR-копию в sRGB;
исходные коды хранятся отдельно и передаются кодировщику полноцветного PNG,
точного TIFF16 или JP2, если профиль поддерживается и размер не изменён.
При выборе PNG8 из 16-битного исходника отсчёты округляются до 8 бит.
Другие lossless-режимы сохраняют пиксели RGBA8, поступившие на кодирование,
без обещания сохранить исходную разрядность, профиль, метаданные или анимацию. Исключение — отдельное
преобразование JPEG → JXL → JPEG: оно работает с исходными байтами, не с RGBA.
Кнопка «Отображение» настраивает общий SDR-показ сравниваемых изображений:
чёрную и белую точки, экспозицию и сглаживание переходов 16→8 бит.
Настройки просмотра не изменяют точные отсчёты, анализ и сохраняемые файлы;
при подтверждённой поддержке браузера PNG16, TIFF16 и JPEG 2000 с точными отсчётами показываются через
Canvas float16, иначе действует прежний RGBA8-путь. Float16 округляет исходные
целые значения и не подтверждает 10-битный сигнал монитора.
Для полноцветных PNG8/PNG16, точного TIFF16 и JP2 с поддерживаемым RGB ICC-профилем просмотр и сравнение
используют преобразованную копию в sRGB. Исходные отсчёты и профиль остаются
отдельно и передаются кодировщику полноцветного PNG, TIFF16 или JP2 без изменения размера;
смена разрядности PNG округляет отсчёты.
JP2 сохраняет их точно только при качестве 100.
Неподдерживаемый профиль выдаёт ошибку; это управление цветом SDR, не HDR.
...
Для точного пути поддерживаются метка sRGB, отсутствие цветовых блоков и RGB ICC-профили с
матрицей и кривыми передачи. Отдельные gAMA/cHRM, другие ICC и HDR-описания
отклоняются с пояснением. Преобразование ICC в sRGB ограничено SDR.
...
Настройки TIFF доступны прямо над изображением: **Разрядность**, **Сжатие**, **Уровень**
...
Режим **Авто** сохраняет 16 бит целочисленного исходника, когда доступен точный
растр. TIFF16 записывается без изменения размеров, с Deflate или без сжатия;
LZW и PackBits для его записи недоступны. Точное чтение TIFF16 ограничено
целыми смежными каналами, независимым alpha, ориентацией 1 и 12 Мп.
Для других разновидностей TIFF16 прежний декодер может дать 8-битный просмотр;
программа явно помечает потерю точности, а при невозможности чтения выдаёт ошибку.
Поддерживаемый RGB ICC классического TIFF16 преобразуется для SDR-сравнения,
а исходные коды и профиль сохраняются при записи TIFF16 без изменения размера.
TIFF16 с ICC вне точного пути, включая BigTIFF16, отклоняется.
При открытии многостраничного TIFF используется первая страница.
Экранный показ строится из 8-битной копии; точные отсчёты доступны инспектору,
гистограмме и метрикам. 10-битный монитор сам по себе не меняет глубину вывода
Canvas. Внешние TIFF-кодеки ZSTD, LZMA, LERC, JBIG и WebP здесь не поддерживаются.
JPEG-варианты ограничены возможностями
...
- JPEG/BMP8/BMP24 заменяют прозрачность заливкой; GIF поддерживает двоичную прозрачность.
  ...
  а исходный поддерживаемый ICC сохраняет в полноцветном PNG без изменения размера;
  точный TIFF16 и JP2 также могут перенести такой профиль.
  Сохранение ICC/HDR через Canvas не гарантируется.
- HEIC, AVIF, JPEG XL и TIFF открываются в отдельных Worker; входной файл — до **256 МиБ**. TIFF16 и целочисленный JPEG XL 16 бит сохраняют RGBA16 отдельно от экранного предпросмотра; HEIC и AVIF выдают RGBA8.
  Поворот и отражение HEIF применяются, 10-битный цвет приводится к 8-битному.
- JP2/J2K читаются отдельным Worker из файла до **64 МиБ**: до **8 Мп** при 8 битах и **4 Мп** при 16 битах на канал. Поддерживаются целые несубдискретизированные Gray/RGB/RGBA. Для JP2 с поддерживаемым RGB ICC просмотр и анализ получают SDR-копию в sRGB; исходные 16-битные отсчёты остаются отдельно и записываются с ICC в JP2 без изменения размера. Экранный резервный растр имеет 8 бит; прочие метаданные не копируются.
