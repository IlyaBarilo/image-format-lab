...
| --- | --- |
| Сравнить форматы | Два или четыре варианта одного исходника, синхронный масштаб и перемещение, шторка для вариантов 1 и 2, сетка пикселей при увеличении, режим 100%, фон для проверки прозрачности |
| Оценить потери | Размер и доля от исходного файла, PSNR RGB на белой подложке, отдельная ошибка прозрачности Δα |
| Исследовать цвет и яркостный сигнал | Панель «Анализ»: гистограммы RGB/alpha, Waveform, RGB Parade и вектороскоп для текущих 2/4 ячеек, общие шкалы и подложка |
...
| Найти отличия в изображении | Карты различий и гистограмма попиксельных ошибок RGB/alpha с исходником; MAE и RMSE по точным RGBA8/16 пикселям, выбор области для всех графиков |
...
| Узнать свойства файла | Паспорт исходника и результатов JPEG/PNG: размеры, разрядность, свойства кодирования, наличие метаданных, bpp и отдельно объём рабочего растра |
...
| --- | --- | --- |
| JPEG | Средствами браузера | Средствами браузера; качество и заливка прозрачности |
| PNG | PNG8 — браузер; PNG16 — точное чтение серых/RGB/RGBA, включая Adam7 | Авто / 8 / 16 бит на канал; PNG16 в исходном размере, точные отсчёты и alpha |
| WebP | Браузер или встроенный libwebp | Обычный WebP — браузер; **WebP lossless** — встроенный libwebp 1.6.0 |
| AVIF | Встроенные libheif / libaom | Встроенный libaom 3.15.0, качество 1–100 |
| JPEG XL | Встроенный libjxl 0.12.0 | **JPEG XL** с качеством 1–100 или отдельный **JPEG XL lossless** |
| ICO | Наибольший PNG внутри ICO — браузер; BMP внутри ICO — встроенный libnsbmp | 7 PNG-размеров: 16, 24, 32, 48, 64, 128, 256 px |
| PNG opt | Как обычный PNG | Через встроенные UPNG и pako, 8 бит на канал |
| GIF | Средствами браузера | Один кадр: собственный кодировщик или gifenc, палитра и двоичная прозрачность |
| BMP | Встроенный libnsbmp: палитры, RGB555/565, RGB24/32, RLE4/8 и битовые маски в пределах поддержки | BMP без сжатия: выбор 24 бит RGB или 32 бит RGBA |
| TIFF | Встроенный libtiff: TIFF/BigTIFF, раздельные каналы, ориентация; UTIF / libjpeg-turbo для JPEG-in-TIFF и lossless JPEG | TIFF RGBA8 без потерь: без сжатия, Deflate или LZW; настройки уровня и предиктора |
...
Для **PNG** над изображением и в пакете доступна **Разрядность: Авто / 8 / 16 бит на канал**.
...
Предел точного пути — **8 Мп**, входной файл — **128 МиБ**.
...
Для **BMP** над изображением доступна **Разрядность**: **24 бита · RGB**
с заливкой прозрачных областей или **32 бита · RGBA** с сохранением прозрачности.
Такая же настройка есть в пакетной конвертации. Оба варианта сохраняются без сжатия.
...
- JPEG/BMP24 заменяют прозрачность заливкой; GIF поддерживает двоичную прозрачность.
...
- Общий предел исходника — **40 мегапикселей**; для точного PNG16 — **8 Мп**.
